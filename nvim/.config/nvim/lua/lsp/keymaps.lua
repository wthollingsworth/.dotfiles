...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(args)
    ...
    keymap.set(
      ...
      vim.tbl_extend("error", bufopts, { desc = "Go to definition" })
    )
    keymap.set(
      "n",
      "gi",
      telescope.lsp_implementations,
      vim.tbl_extend("error", bufopts, { desc = "Go to implementation" })
    )
    keymap.set(
      "n",
      "<leader>wa",
    ...
    keymap.set(
      ...
      vim.tbl_extend("error", bufopts, { desc = "Remove workspace folder" })
    )
    ...
    keymap.set(
      ...
      vim.tbl_extend("error", bufopts, { desc = "Show symbols in the current workspace" })
    )
    keymap.set(
      "n",
      "<leader>ds",
      telescope.lsp_document_symbols,
      vim.tbl_extend("error", bufopts, { desc = "Show symbols in the current buffer" })
    )
    keymap.set(
      "n",
      "<leader>D",
      telescope.lsp_type_definitions,
      vim.tbl_extend("error", bufopts, { desc = "Go to type definition" })
    )
    ...
    keymap.set(
      ...
      vim.tbl_extend("error", bufopts, { desc = "Show code actions" })
    )
    keymap.set(
      "n",
      "<leader>ci",
      telescope.lsp_incoming_calls,
      vim.tbl_extend("error", bufopts, { desc = "Show incoming calls" })
    )
    keymap.set(
      "n",
      "<leader>co",
      telescope.lsp_outgoing_calls,
      vim.tbl_extend("error", bufopts, { desc = "Show outgoing calls" })
    )
    keymap.set("n", "gr", telescope.lsp_references, vim.tbl_extend("error", bufopts, { desc = "Find references" }))
    keymap.set("n", "<leader>f", function()
      vim.lsp.buf.format({ async = true })
    end, vim.tbl_extend("error", bufopts, { desc = "Run formatter" }))
  end,
})
